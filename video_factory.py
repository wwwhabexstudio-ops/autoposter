...
def _prepare_scenes(raw:list[dict],seconds:float,topic:str)->list[dict]:
    ...
    # Keep AI semantic prompts when available; otherwise create a direct prompt from the beat.
    raw_prompts=[str(x.get("visual_prompt") or "") for x in raw]
    if len(raw_prompts)==len(chunks):
        prompts=raw_prompts
    else:
        prompts=[f"cinematic realistic documentary moving shot, directly visualizing: {chunk}" for chunk in chunks]
...
def make_video(audio:str,output:str,width:int,height:int,duration:float|None=None,image:str|None=None,scenes:list[dict]|None=None,topic:str="AutoPoster")->str:
    ...
    if not os.getenv("MOTION_WORKER_URL"):
        raise RuntimeError("Wan2.1 worker is required. Configure MOTION_WORKER_URL before generating a video.")
    ...
    try:
        for i,scene in enumerate(selected,1):
            ...
            # Every beat is generated by the real Wan2.1 T2V worker.
            generate_motion_clip(str(scene["visual_prompt"]),str(generated),width=width,height=height,seconds=round(scene["duration_seconds"]))
